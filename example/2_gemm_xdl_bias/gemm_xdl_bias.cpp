...
#include "device_gemm_xdl_c_shuffle_bias.hpp"
...
using CElementOp = ck::tensor_operation::element_wise::Bias;

// clang-format off
using DeviceGemmInstance = ck::tensor_operation::device::DeviceGemmXdl_C_Shuffle_Bias<
    ADataType,              // ADataType
    BDataType,              // BDataType
    CDataType,              // CDataType
...
int main(int argc, char* argv[])
{
    ...
    auto f_bias_host_tensor_descriptor = [](std::size_t row, std::size_t col) {
        // TODO - stride with {0, 1}
        return HostTensorDescriptor(std::vector<std::size_t>({row, col}),
    ...
    Tensor<CDataType> c0_m_n(f_bias_host_tensor_descriptor(M, N));
